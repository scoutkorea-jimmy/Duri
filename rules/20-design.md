...
>
> #### "두리손잡고" 는 하나의 고유명사입니다
> 한 단어로 취급합니다. 중간에 줄바꿈·`<br>`·공백을 넣지 마세요. `keep-all` 이 어절 분리를 막아주지만,
> **`·` · `-` · `( )` 가 들어간 덩어리는 그 기호에서 끊깁니다.** 그런 곳에는 `.nb`(`white-space:nowrap`)를 씁니다.
>
> ```html
> <span class="nb">직업재활센터 031-853-3360</span>
> <span class="nb">직업재활·주간보호</span> 프로그램
> ```
>
> 전화번호는 예외 없이 `.nb` 로 감쌉니다. `tools/verify-headless.mjs` 가 5개 폭에서
> `.nb` 덩어리가 두 줄로 갈라지는지 검사합니다.
>
> #### 긴 문장은 줄바꿈 위치를 손으로 정합니다
> 자동 줄바꿈에 맡기면 마지막 줄에 한 음절만 남는 경우가 생깁니다(게이트 설명이 1100·900px에서
> "…함께하는 / 길" 로 끊겼음). **의미 단위(쉼표·조사 경계)에 `<br>` 을 넣어 고정**하고,
> 문구를 고칠 때 `<br>` 도 함께 옮기세요.
...
### 칩 · 배지
- `.chip` — 알약 보더 칩(흰 배경). `.chip--soft`는 brand-soft 배경. 페이지 히어로 안에서는 **반투명 흰색 + 보더 없음**으로 자동 변형.
...
### 푸터 유틸 링크 `.foot-reset`
푸터 맨 아래 줄 오른쪽의 "처음 선택 화면 다시 보기". `#c6d8ce`(딥 배경 위 9.5:1) + **밑줄**, 최소 높이 44px. 진입 게이트로 되돌아가는 유일한 UI 경로이므로 **지우지 마세요.**

### 계정 컨트롤 (헤더)
로그아웃 상태: `.btn-login`(흰 알약). 로그인 상태: `.acct-name`(brand-soft 알약, 사람 아이콘 + "관리자") + 로그아웃 버튼. 좁은 폭(≤520px)에서는 라벨 텍스트 숨김.

### ⛔ 반투명 면에는 외곽선을 넣지 않습니다
어두운 배경 위의 반투명 카드·칩·버튼(`.cta-tile`, `.page-hero .pills .chip`, `.gate-close`, 히어로 CTA 버튼)에
**보더를 두지 마세요**(사용자 지시, 2026-08-17). 카드로 읽히게 하려면 **보더 대신 면의 불투명도를 올립니다**
(0.10 → 0.14~0.18). `tools/verify-headless.mjs` 가 "반투명 배경 + 보이는 보더" 조합을 검사합니다.
...
- **다시 열었을 때만** 우상단에 `.gate-close`(흰 보더 알약, 44px 이상)가 붙고 ESC 로도 닫힙니다. 첫 방문에는 없습니다.
  진입 경로는 푸터의 `.foot-reset` 링크와 `?gate=1` / `#gate` → [00-core.md](00-core.md) 4절.
...
단색 딥 그린 면(`--deep-800`) + 반경 `--radius-xl`, 글자 흰색(7.3:1). 내부 `.cta-cards` 는 3열 `.cta-tile`(**반투명 흰 면, 보더 없음** — hover 시 불투명도만 상승). ⚠️ 이 클래스들은 한동안 **정의 없이 사용되어 흰 글자가 흰 배경에 렌더**되고 있었습니다(2026-08-17 수정). 지우지 마세요.

...
10. **"두리손잡고" 는 하나의 고유명사입니다.** 중간에 줄바꿈·공백을 넣지 마세요.
   `·`·`-`·괄호가 들어간 덩어리(전화번호 등)는 `.nb`(`white-space:nowrap`)로 감쌉니다.
   긴 문장은 마지막 줄에 한 음절만 남지 않게 **`<br>` 로 줄바꿈 위치를 손으로 정합니다.**
11. **반투명 면에는 외곽선을 넣지 마세요.** 카드로 읽히게 하려면 보더 대신 면의 불투명도를 올립니다.
12. **새 페이지·새 레이아웃에는 반응형 분기를 반드시 넣습니다.** 2열 이상 그리드는 좁은 폭에서 접혀야 하고,
